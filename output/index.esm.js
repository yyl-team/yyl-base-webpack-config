/*!
 * yyl-react-ts-config-webpack-plugin esm 0.1.0
...
import util, { type, path as path$1 } from 'yyl-util';
...
function initBase(option) {
    ...
    if (env.proxy || env.remote) {
        wConfig.output.publicPath = util.path.join(alias.hostname, alias.basePath, path.relative(alias.root, resolveRoot), '/');
    }
        ...
        wConfig.output.publicPath = util.path.join(alias.basePath, path.relative(alias.root, resolveRoot), '/');
    }
    wConfig.plugins.push(new DefinePlugin({
...
function initModule(op) {
    ...
    const urlLoaderOptions = {
        ...
        outputPath: path$1.relative(resolveRoot, alias.imagesDest),
        publicPath: (function () {
            let r = util.path.join(alias.basePath, path$1.relative(resolveRoot, alias.imagesDest), '/');
    ...
    const wConfig = {
        module: {
            rules: [
                ...
                // lib js
                {
                    test: path$1.join(alias.srcRoot, 'js/lib/'),
                    use: [
                        {
    ...
    if (env.isCommit || env.remote) {
        ...
        wConfig.plugins.push(new MiniCssExtractPlugin({
            filename: util.path.relative(resolveRoot, path$1.join(alias.cssDest, '[name]-[hash:8].css')),
            chunkFilename: util.path.relative(resolveRoot, path$1.join(alias.cssDest, '[name]-[chunkhash:8].css'))
        }));
    }
    ...
    // - css & scss
    // + ts
    const localTsConfigPath = path$1.join(alias.dirname, 'tsconfig.json');
    if (fs.existsSync(localTsConfigPath)) {
        const localPkgPath = path$1.join(alias.dirname, 'package.json');
        const localTsLoaderPath = path$1.join(alias.dirname, 'node_modules', 'ts-loader');
...
export { YylReactTsConfigWebpackPlugin };
